<div align="center">

<img src="https://readme-typing-svg.demolab.com?font=Trebuchet+MS&weight=600&size=34&duration=3500&pause=800&color=00C2FF&center=true&vCenter=true&width=700&lines=Namaste%2C+I'm+Aditya+Dalvi;Backend+Engineer;Performance+Driven+Developer" />

<br/><br/>

<p>
  <a href="https://linkedin.com/in/aditya-dalvi-bkd1">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-blue?style=for-the-badge&logo=linkedin&logoColor=white">
  </a>
  <a href="mailto:adityadalvi157@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact-red?style=for-the-badge&logo=gmail&logoColor=white">
  </a>
  <a href="https://leetcode.com/u/adityadalvi8910/">
    <img src="https://img.shields.io/badge/LeetCode-Solve-orange?style=for-the-badge&logo=leetcode&logoColor=black">
  </a>
  <a href="https://aditya1997-debug.github.io/aditya8/">
    <img src="https://img.shields.io/badge/Website-Portfolio-00C2FF?style=for-the-badge&logo=google-chrome&logoColor=white">
  </a>
</p>
</div>

---

### ⚡ Executive Summary
I am a **Performance-Driven Backend Developer** who transitioned from **Mechanical Engineering**, bringing a rigorous analytical approach to software architecture. I specialize in building high-throughput systems and automating developer workflows.

* **Core Philosophy:** If it's repetitive, automate it. If it's slow, optimize it.
* **Key Achievement:** Reduced feature development time by **99%** through custom internal tooling.
* **Current Focus:** Scaling EdTech infrastructure to support thousands of concurrent evaluations.

---

### 📊 Quantified Impact
| Area | Achievement | Tech Used |
| :--- | :--- | :--- |
| **Development Speed** | Built a **CRUD API Generator** that cut route/service creation time by **99%**. | Node.js, REST API |
| **API Performance** | Refactored legacy queries to achieve up to **90% faster response times**. | MongoDB Indexing, SQL Opt. |
| **Scalability** | Integrated **Redis Caching**, reducing database load and cutting latency by **65%**. | Redis, Express.js |
| **Throughput** | Managed backend for systems handling **5,000+ monthly evaluations**. | Django, PostgreSQL |

---

### 🛠️ Tech Stack & Toolbox

<p align="left">
  <img src="https://skillicons.dev/icons?i=py,js,nodejs,express,django,flask,mongodb,postgres,redis,git&theme=dark" />
</p>

* **Languages:** Python, JavaScript (ES6+), SQL
* **Frameworks:** Node.js, Express, Django, Flask
* **Databases:** MongoDB, PostgreSQL, Redis
* **Architecture:** RESTful APIs, Controller-Service-Repository, Microservices

---

### 📂 Strategic Projects

#### 🧑‍🏫 **mytri – Teacher’s Productivity Suite**
*Architected a lesson planning system aligned with India's NEP 2020.*

* **Problem:** Manual lesson planning overhead
* **Solution:** Automated lesson plans with multimedia
* **Result:** Streamlined workflows across Myelin

#### 📊 **Holistic Progress Card (HPC)**
*Built a competency-based evaluation engine.*

* **Key Feature:** **Predecessor Mapping**
* **Tech Highlight:** Zero-friction **Swagger/OpenAPI**

---

### 💼 Career Snapshot

**Backend Developer @ Myelin** *(Dec 2024 – Present)*  
> Architecting teacher-centric learning ecosystems.

**Associate Software Engineer @ Augmented Transformations** *(Jan 2024 – June 2024)*  
> Improved API performance by **90%**.

**Trainee @ Augmented Transformations** *(Feb 2023 – Oct 2023)*  
> Built exam engines handling 5,000+ evaluations.

---

<div align="center">
  <br/><br/>
  <strong>Let's build something together.</strong>
</div>
